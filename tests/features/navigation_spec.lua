...
describe("with comments in another file", function()
  it("stays in the current file", function()
    local other = helpers.buffer("other.md", { "1", "2", "3" })
    vim.cmd("3Comment")
    helpers.write("in the other file")
    vim.api.nvim_win_set_cursor(0, { 1, 0 })
    vim.cmd("CommentNext")
    vim.cmd("CommentNext")
...
describe("when things change under an open comment", function()
  it("moves on from its line when its comment was deleted", function()
    vim.api.nvim_win_set_cursor(0, { 2, 0 })
    vim.cmd("CommentShow")
  ...
  it("still cycles when the window it came from is closed", function()
    vim.cmd("split")
    vim.api.nvim_win_set_cursor(0, { 2, 0 })
